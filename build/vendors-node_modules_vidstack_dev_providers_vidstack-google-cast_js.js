"use strict";
(globalThis["webpackChunkrrze_video"] = globalThis["webpackChunkrrze_video"] || []).push([["vendors-node_modules_vidstack_dev_providers_vidstack-google-cast_js"],{
...
/* harmony import */ var _chunks_vidstack_DhHK6ZQ_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../chunks/vidstack-DhHK6ZQ-.js */ "./node_modules/vidstack/dev/chunks/vidstack-DhHK6ZQ-.js");
/* harmony import */ var _chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../chunks/vidstack-BdKB12NY2.js */ "./node_modules/vidstack/dev/chunks/vidstack-BdKB12NY2.js");




//#region src/providers/google-cast/media-info.ts
var GoogleCastMediaInfoBuilder = class {
	...
	setStreamType(streamType) {
		if (streamType.includes("live")) this.#info.streamType = chrome.cast.media.StreamType.LIVE;
		else this.#info.streamType = chrome.cast.media.StreamType.BUFFERED;
		return this;
	}
	setTracks(tracks) {
...
//#endregion
//#region src/providers/google-cast/tracks.ts
var GoogleCastTracksManager = class {
	...
	setup() {
		...
		(0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.listenEvent)(this.#ctx.audioTracks, "change", syncRemoteActiveIds);
		(0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.listenEvent)(this.#ctx.textTracks, "mode-change", syncRemoteActiveIds);
		(0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.effect)(this.#syncLocalTracks.bind(this));
	}
	getLocalTextTracks() {
	...
	#getRemoteActiveIds() {
		...
		if (activeLocalTextTracks?.length) {
			...
			if (remoteTextTracks.length) for (const localTrack of activeLocalTextTracks) {
	...
	#syncLocalTracks() {
		...
		const remoteTextTracks = this.#getRemoteTracks(chrome.cast.media.TrackType.TEXT);
		for (const localTrack of localTextTracks) if (!this.#findRemoteTrack(remoteTextTracks, localTrack)) {
			(0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.untrack)(() => this.#onNewLocalTracks?.());
			break;
		}
	}
	syncRemoteTracks(event) {
		...
		for (const remoteAudioTrack of remoteAudioTracks) {
			if (this.#findLocalTrack(localAudioTracks, remoteAudioTrack)) continue;
			...
			this.#ctx.audioTracks[_chunks_vidstack_BzoKHzuo_js__WEBPACK_IMPORTED_MODULE_1__.ListSymbol.add](localAudioTrack, event);
		}
		for (const remoteTextTrack of remoteTextTracks) {
			if (this.#findLocalTrack(localTextTracks, remoteTextTrack)) continue;
	...
	#editTracksInfo(request) {
		const media = (0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.getCastSessionMedia)();
...
//#endregion
//#region src/providers/google-cast/provider.ts
/**
...
var GoogleCastProvider = class {
	...
	scope = (0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.createScope)();
	#player;
	#ctx;
	#tracks;
	#currentSrc = null;
	...
	#currentTime = 0;
	#played = 0;
	#seekableRange = new _chunks_vidstack_DhHK6ZQ_js__WEBPACK_IMPORTED_MODULE_2__.TimeRange(0, 0);
	#timeRAF = new _chunks_vidstack_BzoKHzuo_js__WEBPACK_IMPORTED_MODULE_1__.RAFLoop(this.#onAnimationFrame.bind(this));
	#playerEventHandlers;
	#reloadInfo = null;
	#isIdle = false;
	...
	*/
	get cast() {
		return (0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.getCastContext)();
	}
	/**
	* @see {@link https://developers.google.com/cast/docs/reference/web_sender/cast.framework.CastSession}
	*/
	get session() {
		return (0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.getCastSession)();
	}
	/**
	* @see {@link https://developers.google.com/cast/docs/reference/web_sender/chrome.cast.media.Media}
	*/
	get media() {
		return (0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.getCastSessionMedia)();
	}
	/**
	* Whether the current Google Cast session belongs to this provider.
	*/
	get hasActiveSession() {
		return (0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.hasActiveCastSession)(this.#currentSrc);
	}
	setup() {
	...
	#attachCastContextEventListeners() {
		(0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.listenCastContextEvent)(cast.framework.CastContextEventType.CAST_STATE_CHANGED, this.#onCastStateChange.bind(this));
	}
	#attachCastPlayerEventListeners() {
		const Event = cast.framework.RemotePlayerEventType, handlers = {
		...
		for (const type of (0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.keysOf)(handlers)) this.#player.controller.addEventListener(type, handler);
		(0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.onDispose)(() => {
			for (const type of (0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.keysOf)(handlers)) this.#player.controller.removeEventListener(type, handler);
		});
	}
	async play() {
	...
	setMuted(muted) {
		if (muted && !this.#player.isMuted || !muted && this.#player.isMuted) this.#player.controller?.muteOrUnmute();
	}
	setCurrentTime(time) {
	...
	async loadSource(src) {
		...
		if ((0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.hasActiveCastSession)(src)) {
			this.#resumeSession();
			this.#currentSrc = src;
			return;
		}
		...
		if (errorCode) {
			...
			this.#ctx.notify("error", Error((0,_chunks_vidstack_BdKB12NY2_js__WEBPACK_IMPORTED_MODULE_3__.getCastErrorMessage)(errorCode)));
			return;
		}
		this.#currentSrc = src;
	}
	...
	#reset() {
		if (!this.#reloadInfo) {
			this.#played = 0;
			this.#seekableRange = new _chunks_vidstack_DhHK6ZQ_js__WEBPACK_IMPORTED_MODULE_2__.TimeRange(0, 0);
		}
		this.#timeRAF.stop();
		this.#currentTime = 0;
	...
	#resumeSession() {
		const resumeSessionEvent = new _chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.DOMEvent("resume-session", { detail: this.session });
	...
	#onCastStateChange(data) {
		...
		if (state === "disconnected") this.#disconnectFromReceiver();
	}
	#onMediaLoadedChange(event) {
		...
		Promise.resolve().then(() => {
			if (src !== (0,_chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.peek)(this.#ctx.$state.source) || !this.#player.isMediaLoaded) return;
			this.#reset();
			const duration = this.#player.duration;
			this.#seekableRange = new _chunks_vidstack_DhHK6ZQ_js__WEBPACK_IMPORTED_MODULE_2__.TimeRange(0, duration);
			const detail = {
				provider: this,
				duration,
				buffered: new _chunks_vidstack_DhHK6ZQ_js__WEBPACK_IMPORTED_MODULE_2__.TimeRange(0, 0),
	...
	#getStreamType() {
		return this.#player.mediaInfo?.streamType === chrome.cast.media.StreamType.LIVE ? this.#player.canSeek ? "live:dvr" : "live" : "on-demand";
	}
	#onCurrentTimeChange() {
		...
		if (this.#ctx.$state.seeking()) this.#ctx.notify("seeked", currentTime);
	...
	#onPausedChange(event) {
		const trigger = this.#createEvent(event);
		if (this.#player.isPaused) this.#ctx.notify("pause", void 0, trigger);
		else this.#ctx.notify("play", void 0, trigger);
	}
	#onProgress(event) {
		const detail = {
			seekable: this.#getSeekableRange(),
			buffered: new _chunks_vidstack_DhHK6ZQ_js__WEBPACK_IMPORTED_MODULE_2__.TimeRange(0, this.#played)
	...
	#getSeekableRange() {
		return this.#player.liveSeekableRange ? new _chunks_vidstack_DhHK6ZQ_js__WEBPACK_IMPORTED_MODULE_2__.TimeRange(this.#player.liveSeekableRange.start, this.#player.liveSeekableRange.end) : this.#seekableRange;
	}
	#createEvent(detail) {
		return detail instanceof Event ? detail : new _chunks_vidstack_BwClSYSC_js__WEBPACK_IMPORTED_MODULE_0__.DOMEvent(detail.type, { detail });
	}
	#buildMediaInfo(src) {
...
//# sourceMappingURL=vendors-node_modules_vidstack_dev_providers_vidstack-google-cast_js.js.map?ver=a0e42f2d9b6b39427c19